...
class SetActiveNodeAsOverride(bpy.types.Operator):
    ...
    bl_label = 'Set'
    bl_description = 'Set the active node as the Material Combiner override'
    bl_options = {'UNDO'}
...
class SetOverrideAsActive(bpy.types.Operator):
    ...
    bl_label = 'Select'
    bl_description = 'Select the Material Combiner override as active'
    bl_options = {'UNDO'}
...
class ClearOverride(bpy.types.Operator):
    ...
    bl_label = 'Clear'
    bl_description = 'Clear the current Material Combiner override'
    bl_options = {'UNDO'}
...
class FrameOverride(bpy.types.Operator):
    ...
    bl_label = 'View'
    bl_description = 'Resize view so you can see the override node'

    @classmethod
    def poll(cls, context):
        space_data = context.space_data
        # Must not be in a node_editor and not be inside a group node.
    ...
    def execute(self, context):
        ...
        # Select the override node
        override_node = nodes[mat.smc_override_node_name]
        override_node.select = True
        # View only the selected nodes (which is now only the override node)
